...
public class GenerateReconciliationReportCommandValidatorTests
{
    private readonly GenerateReconciliationReportCommandValidator _validator = new();

    private const string XlsxContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

    private static GenerateReconciliationReportCommand Valid() => new()
    {
        Content = [1, 2, 3],
    ...
    public void ShouldHaveError_WhenFileNameIsEmpty()
        => _validator.Validate(Valid() with { FileName = "" })
            .ShouldHaveErrorFor(nameof(GenerateReconciliationReportCommand.FileName));
