...
public class LeavesReportExporter : IReportExporter<LeavesReportDto>
{
    ...
    [
        ...
        ("Days", 0.08, true),
        ("Status", 0.14, false),
        ("Reviewed By", 0.28, false),
    ];

    public byte[] GeneratePdf(LeavesReportDto report)
    {
        ...
        document.Info.Title = "Leaves Report";
        ...
        gfx.DrawString("Leaves Report", titleFont, XBrushes.Black, new XRect(MarginLeft, y, contentWidth, 26), XStringFormats.TopLeft);
        y += 28;
        gfx.DrawString(report.PeriodLabel, periodFont, XBrushes.Gray, new XRect(MarginLeft, y, contentWidth, 16), XStringFormats.TopLeft);
        y += 26;

        gfx.DrawString(
            $"Total Requests: {report.TotalRequests}    Total Days: {report.TotalDays}    Approved: {report.ApprovedCount}    Rejected: {report.RejectedCount}    Pending: {report.PendingCount}",
        ...
        {
            gfx.DrawString("No leave requests in this period.", bodyFont, XBrushes.Gray, new XRect(MarginLeft, y, contentWidth, RowHeight), XStringFormats.TopLeft);
            y += RowHeight;
        }
        ...
        foreach (var row in report.Rows)
        {
            ...
            var values = new[]
            {
                row.EmployeeName, row.StartDate.ToString("yyyy-MM-dd"), row.EndDate.ToString("yyyy-MM-dd"),
                row.Days.ToString(), row.Status, row.ReviewedByName ?? "—"
            };
    ...
    public byte[] GenerateExcel(LeavesReportDto report)
    {
        using var workbook = new XLWorkbook();
        var sheet = workbook.Worksheets.Add("Leaves Report");

        var row = 1;
        sheet.Cell(row, 1).Value = "Leaves Report";
        ...
        SummaryRow("Total Requests", report.TotalRequests);
        SummaryRow("Total Days", report.TotalDays);
        SummaryRow("Approved", report.ApprovedCount);
        SummaryRow("Rejected", report.RejectedCount);
        SummaryRow("Pending", report.PendingCount);
        row++;

        var headerRow = row;
        string[] headers = ["Employee", "Start Date", "End Date", "Days", "Status", "Reviewed By"];
        ...
        foreach (var r in report.Rows)
        {
            ...
            startCell.Value = r.StartDate.ToDateTime(TimeOnly.MinValue);
            startCell.Style.DateFormat.Format = "yyyy-mm-dd";
            var endCell = sheet.Cell(row, 3);
            endCell.Value = r.EndDate.ToDateTime(TimeOnly.MinValue);
            endCell.Style.DateFormat.Format = "yyyy-mm-dd";
            sheet.Cell(row, 4).Value = r.Days;
            sheet.Cell(row, 5).Value = r.Status;
            sheet.Cell(row, 6).Value = r.ReviewedByName ?? "—";
            row++;
        }
