...
public class SuppliersReportExporter : IReportExporter<SuppliersReportDto>
{
    ...
    public byte[] GeneratePdf(SuppliersReportDto report)
    {
        ...
        gfx.DrawString("Suppliers Report", titleFont, XBrushes.Black, new XRect(MarginLeft, y, contentWidth, 26), XStringFormats.TopLeft);
        y += 28;
        gfx.DrawString(report.PeriodLabel, periodFont, XBrushes.Gray, new XRect(MarginLeft, y, contentWidth, 16), XStringFormats.TopLeft);
        y += 26;

        gfx.DrawString(
            $"Suppliers: {report.TotalSuppliers}    Transactions: {report.TotalTransactions}    Total Quantity Received: {report.TotalQuantityReceived}",
        ...
        {
            gfx.DrawString("No supplier activity in this period.", bodyFont, XBrushes.Gray, new XRect(MarginLeft, y, contentWidth, RowHeight), XStringFormats.TopLeft);
            y += RowHeight;
        }
        ...
        foreach (var row in report.Rows)
        {
            ...
            var x = MarginLeft;
            var values = new[] { row.SupplierName, row.ItemName, row.Quantity.ToString(), row.StockDate.ToString("yyyy-MM-dd") };
    ...
    public byte[] GenerateExcel(SuppliersReportDto report)
    {
        using var workbook = new XLWorkbook();
        var sheet = workbook.Worksheets.Add("Suppliers Report");

        var row = 1;
        sheet.Cell(row, 1).Value = "Suppliers Report";
        ...
        sheet.Cell(row, 1).Value = "Suppliers";
        sheet.Cell(row, 1).Style.Font.Bold = true;
        sheet.Cell(row, 2).Value = report.TotalSuppliers;
        row++;
        sheet.Cell(row, 1).Value = "Transactions";
        sheet.Cell(row, 1).Style.Font.Bold = true;
        sheet.Cell(row, 2).Value = report.TotalTransactions;
        row++;
        sheet.Cell(row, 1).Value = "Total Quantity Received";
        sheet.Cell(row, 1).Style.Font.Bold = true;
        sheet.Cell(row, 2).Value = report.TotalQuantityReceived;
        row += 2;

        var headerRow = row;
        string[] headers = ["Supplier", "Item", "Quantity", "Date"];
        ...
        {
            sheet.Cell(row, 1).Value = r.SupplierName;
            sheet.Cell(row, 2).Value = r.ItemName;
            sheet.Cell(row, 3).Value = r.Quantity;
            var dateCell = sheet.Cell(row, 4);
            dateCell.Value = r.StockDate.ToDateTime(TimeOnly.MinValue);
