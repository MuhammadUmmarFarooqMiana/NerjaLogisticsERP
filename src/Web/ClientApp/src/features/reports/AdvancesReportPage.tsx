...
import type { AdvancesReportRowDto } from '../../api/generated/apiSlice';
import { advancesReportExportUrl, useGetApiReportsAdvancesQuery } from '../../api/reportsApi';
...
export default function AdvancesReportPage() {
  ...
  const { data: report, isLoading, error } = useGetApiReportsAdvancesQuery(periodArgs, { skip: !customReady });

  const columns: ColumnDef<AdvancesReportRowDto, unknown>[] = [
  ...
  return (
    <Box>
      <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
        ...
        <ReportActionButtons
          disabled={!report}
          exportUrl={(format) => advancesReportExportUrl({ ...periodArgs, format })}
          fallbackFileName="advances-report"
        />
      </Stack>
