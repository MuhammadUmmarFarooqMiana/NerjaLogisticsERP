...
public class CorrectDailyOrderCommandHandler : IRequestHandler<CorrectDailyOrderCommand>
{
    private readonly IApplicationDbContext _context;
    private readonly IUser _currentUser;

    public CorrectDailyOrderCommandHandler(IApplicationDbContext context, IUser currentUser)
    { _context = context; _currentUser = currentUser; }

    public async Task Handle(CorrectDailyOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        order.Correct(_currentUser.Id!.Value, request.CompletedOrders, request.Reason);
